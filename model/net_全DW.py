...
class ASKCResUNet(nn.Module):
    def __init__(self, in_channels=1, layers=[1,1,1], channels=[8,16,32,64], fuse_mode='AsymBi', tiny=False, classes=1,
        ...
        if tiny:  # 默认是False
            ...
            nn.Conv2d(in_channels,stem_width,kernel_size=1),
            norm_layer(stem_width,momentum=self.momentum),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=stem_width,out_channels=stem_width, kernel_size=3, stride=1,padding=1,groups=stem_width, bias=False),
            norm_layer(stem_width,momentum=self.momentum),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=stem_width,out_channels=stem_width , kernel_size=3, stride=1,padding=1,groups=stem_width, bias=False),
            nn.Conv2d(stem_width,stem_width*2,kernel_size=1),
            norm_layer(stem_width * 2,momentum=self.momentum),
            nn.ReLU(inplace=True),
